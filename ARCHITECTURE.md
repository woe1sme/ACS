...
**Наблюдаемость.** Логи — Serilog (консоль + OTLP), трейсы и метрики — OpenTelemetry: ASP.NET Core, HttpClient/gRPC, MassTransit, Npgsql. W3C trace context проходит через REST, gRPC и сообщения, поэтому путь события через все сервисы собирается в один трейс. Метрики также отдаются в формате Prometheus на `/metrics`. Для локального просмотра в compose есть Aspire Dashboard.
